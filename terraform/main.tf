...
# ---------------------------------------------------------------------------
# /documents/{doc_id}  (DELETE) — removes a document's index from DynamoDB
# ---------------------------------------------------------------------------

resource "aws_lambda_function" "delete_doc" {
  function_name    = "${var.project_name}-delete-doc"
  role             = aws_iam_role.lambda_exec.arn
  filename         = data.archive_file.lambda_zip.output_path
  source_code_hash = data.archive_file.lambda_zip.output_base64sha256
  ...
  depends_on = [aws_cloudwatch_log_group.delete_doc_logs]
}

resource "aws_cloudwatch_log_group" "delete_doc_logs" {
  name              = "/aws/lambda/${var.project_name}-delete-doc"
  retention_in_days = var.log_retention_days
}

# /documents/{doc_id} path parameter resource
...
resource "aws_api_gateway_integration" "delete_doc_lambda" {
  rest_api_id             = aws_api_gateway_rest_api.rag_api.id
  resource_id             = aws_api_gateway_resource.document_by_id.id
  http_method             = aws_api_gateway_method.document_delete.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.delete_doc.invoke_arn
}

resource "aws_lambda_permission" "delete_doc_apigw" {
  statement_id  = "AllowAPIGatewayInvoke"
  action        = "lambda:InvokeFunction"
...
resource "aws_api_gateway_deployment" "rag_api" {
  ...
  triggers = {
    redeployment = sha1(jsonencode([
      ...
      aws_api_gateway_integration.delete_doc_lambda,
      ...
      aws_api_gateway_integration.options_document_by_id,
    ]))
  }
